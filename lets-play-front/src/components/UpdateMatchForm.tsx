...
function UpdateMatchForm({
  ...
  toggleEditMode,
  handle,
}: UpdateMatchFormProps) {
  const [participant, setParticipant] = useState("");
  // const [updatedMatch, setUpdatedMatch] = useState<MatchProps>(matchData);

  const formattedDate = dayjs(matchData.date, "DD/MM/YYYY").format(
    "YYYY-MM-DD"
  );
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    await http.put(`/matches/${matchData.id}`, {
      ...matchData,
      gameTitle: matchData.game,
    ...
    <>
      ...
      >
        <div className="w-full flex items-center">
          <div className="w-full flex gap-3 justify-center">
            <div className="bg-input p-1 rounded-lg">
              <input
                className="text-primary font-outline-0 text-xl bg-panel rounded-lg p-1"
                value={matchData.game}
                onChange={(event) => handle.editGameName(event.target.value)}
              />
            </div>
            <button className="w-8 h-8">
              <img
                className="hover:drop-shadow-tertiary"
                src={trophyIcon}
                alt=""
              />
            </button>
          ...
          <button className="flex self-end" onClick={handleSubmit}>
            <img
              className="h-12 right-[7%] hover:drop-shadow-primary hover:bg-highlight rounded-2xl p-2"
              src={pencil}
              alt=""
            />
          </button>
            ...
            <div className="w-full h-fit flex self-start items-start pt-6 gap-4">
              ...
              <div className="w-3/4 h-fit flex flex-col">
                ...
                <div className="w-full h-full rounded-lg bg-input p-2">
                  <textarea
                    className="w-full h-36 max-h-36 flex text-left p-1 rounded-lg bg-panel text-primary"
                    value={matchData.description}
                    onChange={(event) =>
                      handle.editDescription(event.target.value)
                    }
                  />
                </div>
              </div>
            ...
            <div className="flex gap-2">
              <div className="w-full max-h-44 flex flex-col p-2 gap-4 justify-center overflow-hidden hide-scroll-bar">
                <div className="w-full flex items-center justify-around p-2 gap-2 bg-input rounded-lg">
                  <input
                    className="w-full bg-panel p-2 rounded-lg"
                    type="date"
                    value={formattedDate}
                    onChange={(event) => handle.editDate(event.target.value)}
                  />
                  -
                  <input
                    className="w-full bg-panel p-2 rounded-lg"
                    ...
                    onChange={(event) => handle.editTime(event.target.value)}
                  />
                </div>
                <div className="w-full flex flex-col bg-input p-4 rounded-lg">
                  <label className="text-white font-outline-1">
                    Voice Channel
                  </label>
                  <input
                    className="w-full p-2 rounded-lg bg-panel text-primary"
                    type="text"
                    value={matchData.voiceChannel}
                    onChange={(event) =>
                      handle.editVoiceChannel(event.target.value)
                    }
                  />
                </div>
              </div>
          ...
          <div className="h-full flex flex-col overflow-scroll hide-scroll-bar pr-2">
            ...
            <div className="w-full flex items-center justify-between gap-2">
              ...
              <button
                ...
                onClick={() => handle.addParticipant(participant)}
                type="button"
              >
                <img
                  className="w-9 h-9"
              ...
              {matchData.participants.map((user, index) => (
                ...
                >
                  ...
                  <button
                    ...
                    type="button"
                    onClick={() => handle.removeParticipant(user)}
                  >
                    <img
        ...
        <div className="w-fit flex gap-4 pb-2">
          <button
            type="submit"
            className="w-30 h-14 text-white bg-primary rounded-md my-2 p-2 hover:drop-shadow-secondary"
