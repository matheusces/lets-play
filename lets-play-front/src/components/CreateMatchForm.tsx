...
function CreateMatchForm({ toggleCreateMatchForm }: CreateMatchFormProps) {
  ...
  async function handleSubmitForm(event: FormEvent<HTMLFormElement>) {
    ...
    await http.post("/matches", formData);
    console.log("Formulário enviado");

    toggleCreateMatchForm(false);
    clearForm();
  }
      ...
      >
        ...
        <div className="w-full flex justify-evenly gap-2">
          <div className="flex flex-col">
            ...
            <input
              className="w-64 h-10 rounded-lg bg-input p-2 text-secondary text-2xl focus:border-secondary"
              type="date"
              value={selectedDate}
          ...
          <div className="flex flex-col">
            ...
            <input
              className="w-64 h-10 rounded-lg bg-input p-2 focus:border-secondary"
              type="time"
              value={selectedTime}
        ...
        <div className="w-11/12 flex flex-col items-start self-start mx-8">
          ...
          <textarea
            maxLength={90}
            className="w-full h-32 max-h-32 hide-scroll-bar bg-input p-2 rounded-lg"
            value={description}
        ...
        <div className="flex gap-20 mt-8">
          <button
            type="submit"
            className="bg-primary text-white p-2 rounded-lg hover:drop-shadow-secondary"
          >
            Confirmar
          </button>
          <button
            type="button"
            className="bg-white text-primary p-2 rounded-lg hover:drop-shadow-secondary"
            onClick={() => toggleCreateMatchForm(false)}
          >
            Cancelar
          </button>
        </div>
