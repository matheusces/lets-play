...
function CreateLeagueForm({
  ...
  const [isParticipantsManagerActive, setIsParticipantsManagerActive] =
    useState(false);
  ...
  function groupParticipantsByTeam(participants: ParticipantProps[]): {
    [team: string]: string[];
  } {
  ...
  function handleRemoveParticipant(user: string) {
    setParticipants((prevParticipants) =>
      prevParticipants.filter((participant) => participant.name !== user)
    );
  }
  ...
  function clearTeams() {
    ...
    participants.map(
      (participant, index) =>
        (updatedParticipants[index] = { ...participant, team: "Time 1" })
    );
  ...
  async function handleSubmitForm(event: FormEvent<HTMLFormElement>) {
    ...
    // await http.post("/leagues", formData);
  ...
  useEffect(() => {
    const numberOfTeams = Object.keys(
      groupParticipantsByTeam(participants)
    ).length;
    numberOfTeams === 0 || numberOfTeams === 1
      ? setLeagueSize(participants.length)
      : setLeagueSize(numberOfTeams);
  ...
  return (
    <form
      ...
      className="w-[42rem] h-[35rem] bg-form absolute z-10 left-1/4 z-1 flex flex-col gap-3 text-secondary items-center text-3xl px-12 py-5"
    >
      <h1>Criar Liga</h1>
      <button
        type="button"
        onClick={handleToggleDropdown}
        className="w-[35rem] h-28 flex justify-between items-center bg-input text-white text-center rounded-lg font-outline-1 border focus-within:border-secondary"
      >
        <img
          src={selectedImage === undefined ? noImage : selectedImage}
          className="w-28 h-28 bg-img border border-secondary rounded-lg"
        />
        <input
          className="w-96 text-center p-2 bg-input text-ellipsis focus:outline-0"
      ...
      <div className="w-full flex flex-col self-start">
        ...
        <input
          className="w-full h-10 self-start rounded-lg bg-input p-2 text-white font-outline-1 text-2xl focus:border-secondary"
          type="text"
          placeholder="Título"
          value={title}
      ...
      <div className="w-full h-full flex justify-between gap-2">
        <div className="h-full flex flex-col gap-7">
          <div className="flex flex-col">
            ...
            <input
              disabled
              className="w-40 h-10 self-start rounded-lg bg-input p-2 text-secondary text-2xl focus:border-secondary"
              value={leagueSize}
            />
          </div>
          ...
          <div className="w-fit flex flex-col gap-2 px-4">
            <button
              className="w-36 h-14 bg-primary p-2 text-base text-white rounded-lg hover:drop-shadow-secondary"
              type="submit"
            >
              Confirmar
            </button>
            <button
              className="w-36 h-14 bg-white p-2 text-base text-primary rounded-lg hover:drop-shadow-secondary"
              type="button"
              onClick={handleToggleCreateLeagueForm}
            >
              Cancelar
            </button>
          </div>
        ...
        <div className="flex flex-col">
          ...
          <div className="w-full flex items-center justify-between gap-2">
            <input
              className="w-full h-10 rounded-lg bg-input p-2 text-ellipsis text-secondary text-lg focus:border-secondary"
              type="text"
            ...
            />
            <button
              className="w-fit h-fit rounded-xl hover:drop-shadow-secondary"
          ...
          <div className="w-80 h-40 max-h-40 overflow-scroll hide-scroll-bar flex flex-col mt-4 rounded-lg bg-input p-2 text-secondary text-2xl focus:border-secondary">
            {participants.map((user, index) => (
              ...
              >
                <button
                  className="w-full flex flex-row items-center gap-2 hover:drop-shadow-secondary hover:cursor-pointer"
                  type="button"
                  onClick={handleToggleParticipantsManager}
                >
                  <img
                    className="p-1 w-10 h-10"
                    src={hourglassIcon}
                    alt="ampulheta"
                    title="aceitação pendente"
                  />
                  <span>{user.name}</span>
                </button>

                <button
                  className="w-fit h-fit hover:drop-shadow-secondary hover:cursor-pointer"
                  type="button"
                  onClick={() => handleRemoveParticipant(user.name)}
                >
                  <img
                    className="w-8 h-7 self-end"
                    src={wasteBinIcon}
        ...
        {isParticipantsManagerActive && (
          <>
            <div className="w-[42rem] h-[35rem] max-h-[35rem] flex flex-col absolute left-0 bottom-0 bg-form p-6 z-10">
              <h1 className="w-full flex justify-center pt-2 pb-4">
              ...
              <div className="flex flex-col">
                <div className="flex gap-4 text-primary mb-8">
                  ...
                  <input
                    ...
                    onChange={handleSoloCheckChange}
                  />
                </div>
                ...
                <div className="max-h-72 border border-primary p-2 overflow-scroll hide-scroll-bar">
                  {participants.map((participant, participantIndex) => (
                    <div
                      ...
                      className="grid grid-cols-2 p-1 rounded-lg items-center hover:bg-highlight hover:drop-shadow-primary"
                    >
                      <span>{participant.name}</span>
                      <select
                        className="w-86 h-10 self-start rounded-lg bg-input p-2 text-secondary text-2xl focus:border-secondary"
                        value={participant.team}
                        onChange={(e) =>
                          handleChangeTeam(e.target.value, participantIndex)
                        }
                        name="team"
                        id="team"
                        disabled={isSoloChecked}
                      >
                ...
                <button
                  className="w-fit h-fit self-center mt-10 p-2 rounded-xl text-white bg-primary"
                  type="button"
                  onClick={handleDone}
                >
                  Feito
