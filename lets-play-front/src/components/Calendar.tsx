...
function Calendar() {
  ...
  function getDaysOfTheMonth(){
    const date = dayjs(`${dayjs().year()}-${months.indexOf(selectedMonth) > 9 ? months.indexOf(selectedMonth) : `0${months.indexOf(selectedMonth) + 1}`}`);
  ...
  return (
    <>
      <div className="flex mt-9 mb-8 text-white items-center gap-11 text-3xl">
        ...
        <button className="w-80 h-20 bg-primary rounded-xl text-center hover:drop-shadow-secondary" onClick={handleToggleDropdown}>{selectedMonth}</button>
          ...
          <div className='w-[20rem] max-h-48 overflow-scroll hide-scroll-bar absolute bg-panel border border-input rounded-br-lg rounded-bl-lg top-[29%] left-[35.8%]'>
            <ul className='flex flex-col items-center gap-2'>
              {months.map((month: string, index) => (
                ...
                >
                  <button className='flex w-full justify-center' onClick={() => handleChangeMonth(month)}>
                   {month}
                  </button>
                </li>
              ))}
