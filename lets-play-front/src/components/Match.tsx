...
function Match({ match, handleToggleMatchList }: MatchComponentProps) {
  ...
  function addParticipant(participant: string) {
    setMatchData((prevMatchData) => ({
      ...prevMatchData,
      participants: [...prevMatchData.participants, participant],
    }));
  }
  ...
  function removeParticipant(user: string) {
    setMatchData((prevMatchData) => ({
      ...prevMatchData,
      participants: prevMatchData.participants.filter(
      ...
      {isEditMode ? (
        ...
        <div className="w-full h-full flex flex-col items-center px-8">
          <div className="w-full flex items-center">
            <button
              className="w-30 h-10 text-white bg-primary rounded-md my-2 p-2 hover:drop-shadow-primary"
              onClick={handleToggleMatchList}
            >
              Voltar
            </button>
              ...
              {(isTournament || isLeague) && (
                <button className="w-8 h-8">
                  <img
                    className={
                      isTournament
                        ? "hover:drop-shadow-tertiary"
                        : "hover:drop-shadow-secondary"
                    }
                    src={isTournament ? trophyIcon : trophyIcon2}
                    alt=""
                  />
                </button>
              )}
            ...
            <div className="w-8/12 h-full flex flex-col gap-3">
              <div className="w-full h-fit flex self-start items-start pt-6 gap-4">
                ...
                <div className="w-3/4 h-fit flex flex-col">
                  <span>Descrição</span>
                  <p className="w-full h-40 p-4 rounded-lg bg-input text-white font-outline-1">
              ...
              <div className="w-full flex gap-2">
                <div className=" w-full flex flex-col gap-2">
                  <div className="w-full flex items-center justify-around p-2 gap-4 bg-input rounded-lg">
                    <span className="w-full bg-panel p-2 rounded-lg">
                      {matchData.date}
                    </span>
                    -
                    <span className="w-full bg-panel p-2 rounded-lg">
                  ...
                  <div className="w-full flex flex-col self-start bg-input p-4 rounded-lg">
                    ...
                    <span className="p-2 bg-panel rounded-lg text-input hover:cursor-default">
                      {matchData.voiceChannel}
                    </span>
                  </div>
                </div>
              </div>
